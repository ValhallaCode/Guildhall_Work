...
Wander::Wander(tinyxml2::XMLElement& element)
	:m_pathGenerator(nullptr)
{
	m_name = element.Name();
}
...
void Wander::ManagePathToTraverse()
{
	...
	if (m_pathGenerator == nullptr)
	{
		while(!m_didIAct)
		{
			Tile goal = m_actingCharacter->m_currentMap->GetRandomTileThatIsNotSolid();
			IntVector2 characterPos = m_actingCharacter->GetPositionInMap();
			IntVector2 goalLocation = goal.GetPositionInMap();
